...
-- Получить список пользователей и клиентов, удалив одинаковых пользователей и клиентов.
...
-- проверьте присутствует ли буква А в последовательности a b c d (оператор in)

select 'A' in ('A', 'B', 'C', 'D') as result; -- true(1), false(0)

-- отсутствует ли буква А

select 'A' not in ('A', 'B', 'C', 'D') as result; -- true(1), false(0)

-- Получить столбцы из таблички clients в котором первое имя является набором значений.
